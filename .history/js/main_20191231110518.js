/**
 * @author: yeheng
 * @param: id: canvas画布 sustained: 游戏时长 interval: 金币间隔 amount: 金币数量 fn: 接口
 * @event: Game游戏主框架 Tips开始前倒计时 Petal金币 End结束
 * @description: 游戏使用面向对象编写，可高度复用，所有参数均可灵活配置
 */

var bizSourceCate = PinganHealth.getQueryValue('bizSourceCate') || 1;
var bizSourceType = PinganHealth.getQueryValue('bizSourceType') || ((!IsPAH ? 2 : 1)); //1直接／2分享
var bizSourceId = PinganHealth.getQueryValue('bizSourceId') || 120; //后台给
sessionStorage.setItem('bizSourceInfos', bizSourceCate + '|' + bizSourceType + '|' + bizSourceId);

var re_from = PinganHealth.getQueryValue('re_from') || 'act_newyear_hby2020';
var channel = PinganHealth.getQueryValue('re_from') || 'act_newyear_hby2020';
var order_from = PinganHealth.getQueryValue('order_from') || 'act_newyear_hby2020';
var sharePhone = PinganHealth.getQueryValue('phone') || '';

var platform = 3;
var joinChannle = 1

if (IsPAH) {
  isLoginStatus(true);
  setTimeout(function () {
    if (loginStatus) {
      getThis()
      getInfo()
      var game = new Game({ id: 'canvas', sustained: 13500, interval: 200, amount: 50, fn: getGold })
      $('.close').on('click', function () {
        if (game.flag) { // 如果为true 游戏已经结束
          PinganHealth.goCommonRouter('pahealth://native/tabBarWithBackRootSceneCMD?selectedIndex=0');
          return
        }
        var num = game.storage || 0
        getGold(num).then(res => {
          PinganHealth.goCommonRouter('pahealth://native/tabBarWithBackRootSceneCMD?selectedIndex=0');
        }).catch(err => {
          PinganHealth.toast(err.msg)
        })
      })
    } else {
      GetLogin1()
    }
  },250)
} else {}

function getGold(num) {
  var obj = {
      bizSourceCate: bizSourceCate,
      bizSourceType: bizSourceType,
      bizSourceId: bizSourceId,
      activityId: bizSourceId,
      userId: infos.ab,
      accessToken: infos.sb,
      joinChannle: 3,
      platform: 3,
      channel: channel,
      blackBox: fp,
      blackBoxType: "SPARTA",
      taskId: 2,
      taskKey: num
    }
    return new Promise((resolve, reject) => {
      getData('/activityMapi/finishActivityTask', obj, function (res) {
        if (res.code == 1) {
          resolve(res)
        } else {
          reject(res)
        }
      })
    })
}

function getThis() {
  var obj = {
    bizSourceCate: bizSourceCate,
    bizSourceType: bizSourceType,
    bizSourceId: bizSourceId,
    activityId: bizSourceId,
    userId: infos.ab,
    accessToken: infos.sb,
    joinChannle: 3,
    platform: 3,
    channel: channel,
    blackBox: fp,
    blackBoxType: "SPARTA",
    taskId: 2
  }

  getData('/activityMapi/queryUserTaskStatus', obj, function (res) {
    if (res.code == 1) {
      if (res.data.status == 'Y') {
        var url = urlDomain + '/activity/2020/newYearAct/index.html' + location.search;
        locationReplace(url)
      }
    } else {
      PinganHealth.toast(res.msg)
    }
  })
}

function locationReplace (url) {
  setTimeout(() => {
    if (window.history.replaceState) {
      window.history.replaceState(null, document.title, url);
      window.history.go(0);
    } else {
      window.location.replace(url);
    }
  }, 200)
}

// 查询活动
function getInfo() {
  var obj = {
    bizSourceCate: bizSourceCate,
    bizSourceType: bizSourceType,
    bizSourceId: bizSourceId,
    activityId: bizSourceId,
    joinChannle: 3,
    platform: 3,
    channel: channel
  }

  getData('/activityMapi/activityInfo', obj, function (res) {
    if (res.code == 1) {
      var flag= new Date().getTime() - new Date(res.data.currentTime).getTime() < 86400000
      if (!flag) {
        var url = urlDomain + '/activity/2020/newYearAct/index.html' + location.search;
        locationReplace(url)
      }
    }
  })
}


// app内统一登录
function GetLogin1() {
  isLogin(false, false, function (result) {
    if (result.code != 1) {
      PinganHealth.toast(result.msg)
    }
  })
}

function getData(url, params, callback) {
  $('#sync-mini').css('display', 'block');
  $.ajax({
    url: hostName + url + '.json?appVersionName=1.3.0&deviceId=5a4c935cbb6ff6ca&deviceType=SM-G9300&timestamp=' + new Date().getTime() + '&platform=' + platform + '&app_key=PAHealth&osversion=100&version=1.3.0&resolution=1440x2560&screenSize=22&netType=1',
    dataType: 'json',
    type: 'POST',
    data: params,
    success: function (result) {
      callback && callback(result);
      $('#sync-mini').css('display', 'none');
    },
    error: function (result) {
      PinganHealth.toast(result.msg);
      $('#sync-mini').css('display', 'none');
    }
  });
}

// Cookie.remove('PAH_SO');
// GetLogin()
// function GetLogin() {
//   isLogin(false, true, function (result) {
//     if (result.code == 1) {
//       // getGold()
//       getThis()
//       var game = new Game({ id: 'canvas', sustained: 13500, interval: 200, amount: 50, fn: getGold })
//     } else {
//       PinganHealth.toast(result.msg)
//     }
//   });
// }
